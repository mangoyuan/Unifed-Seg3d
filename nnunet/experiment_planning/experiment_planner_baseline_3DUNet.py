    ...
    def plan_experiment(self):
        ...
        def get_properties_for_stage(current_spacing, original_spacing, original_shape, num_cases,
            ...
            plan = {
                ...
                'patch_size': np.array([96, 128, 128]),
        ...
        # 获取spacing的中位数大小，然后根据目标spacing计算新的shape大小
        target_spacing = self.get_target_spacing()
        new_shapes = [np.array(i) / target_spacing * np.array(j) for i, j in zip(spacings, sizes)]

        max_spacing_axis = np.argmax(target_spacing)
        remaining_axes = [i for i in list(range(3)) if i != max_spacing_axis]
        # 把最大的axis提到最前，假设之前为[0, 1, 2]，其中axis=2最大，transpose_forward=[2, 0, 1]
        # 反过来的时候，transpose_backward=[1, 2, 0]
        self.transpose_forward = [max_spacing_axis] + remaining_axes
        # 这么magic的写法其实就是找到transpose_forward中0, 1, 2各自的位置
