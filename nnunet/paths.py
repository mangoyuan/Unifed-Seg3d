...
# Mapping the case_id to the specified chosen modality.
caseid_modal_path = 'OMBTS/caseid_modal.yaml'  # need to modify
with open(caseid_modal_path, 'r') as f:
    caseid2modal = yaml.load(f, Loader=yaml.FullLoader)
caseid2modal['train'].update(caseid2modal['test'])
caseid2modal = caseid2modal['train']

modals = ['FLAIR', 'T1w', 't1gd', 'T2w']
modal2index = {m: i for i, m in enumerate(modals)}
m_dim = len(modals)
print(modal2index, m_dim)
