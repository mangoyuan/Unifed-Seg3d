...
def get_case_identifier(case):
    case_identifier = case[0].split("/")[-1].split(".nii.gz")[0][:-5]  # 去除000x的后缀
...
def load_case_from_list_of_files(data_files, seg_file=None):
    """ data_files: [000, 001, 002, ...], seg_file: label """
...
class ImageCropper(object):
    ...
    def crop(data, properties, seg=None):
        ...
        data, seg, bbox = crop_to_nonzero(data, seg, nonzero_label=-1)  # 背景类为0，非零类为-1
    ...
    def load_crop_save(self, case, case_identifier, overwrite_existing=False):
        """case: [000, 001, .., label], case_id: 例如BRATS_001"""
        ...
        if overwrite_existing \
            ...
            all_data = np.vstack((data, seg))  # 把label当作最后一个组装起来，然后进行压缩保存成npz的形式
