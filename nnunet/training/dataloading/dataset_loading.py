...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            modal = caseid2modal[i]
            m_index = modal2index[modal]
            m_indexs.append(m_index)
        ...
        data = np.vstack(data)
        seg = np.vstack(seg)
        m_indexs = np.array(m_indexs)
        # print(selected_keys, m_indexs)
        return {'data':data, 'seg':seg, 'properties':case_properties, 'keys': selected_keys, 'mindex': m_indexs}
...
if __name__ == "__main__":
    p = 'C:/Users/user/workspace/MMBT/nnUNet_preprocessed/Task20_MixBrainTumour/nnUNet_stage0'
    dataset = load_dataset(p)
    with open("C:/Users/user/workspace/MMBT/nnUNet_preprocessed/Task20_MixBrainTumour/nnUNetPlans_plans_3D.pkl", 'rb') as f:
        plans = pickle.load(f)
    unpack_dataset(p)
    dl = DataLoader3D(dataset, (64, 128, 128), (64, 128, 128), 2, oversample_foreground_percent=0.33)
    from mixmodal.vis import plot3d
    data = next(dl)
    print(data['data'].shape, data['mindex'], data['data'][0][:, 30, 64, 64])
